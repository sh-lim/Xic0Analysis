...
//___________________________________________________________________
AliAnalysisTaskSEXic0Semileptonic::AliAnalysisTaskSEXic0Semileptonic( const AliAnalysisTaskSEXic0Semileptonic& ap ) :
  AliAnalysisTaskSE("AliAnalysisTaskSEXic0Semileptonic"),
  fOption(ap.fOption) {;}

//___________________________________________________________________
AliAnalysisTaskSEXic0Semileptonic& AliAnalysisTaskSEXic0Semileptonic::operator = ( const AliAnalysisTaskSEXic0Semileptonic& ap ) {
  // assignment operator
...
Bool_t AliAnalysisTaskSEXic0Semileptonic::FilterElectron(AliAODTrack *etrk, Double_t &minmass, Double_t &minmass_ss, Bool_t IsSameSign, Bool_t IsLoose, Int_t FillHistosFlag){
...
void AliAnalysisTaskSEXic0Semileptonic::FillMCXic0(AliAODMCParticle *mcpart){
  ...
  if(motherxic>0 && e_flag && xi_flag){
    ...
    if(((TMath::Abs(MCMother->GetPdgCode()))==5132) || ((TMath::Abs(MCMother->GetPdgCode()))==5232)){  //Xib
      if(MCe->Charge()>0 && MCcasc->Charge()<0 && fabs(mcpart->Y())<0.5) fHistos->FillTH1("hXic0PtFromBottom1",mcpart->Pt());
      if(MCe->Charge()<0 && MCcasc->Charge()>0 && fabs(mcpart->Y())<0.5) fHistos->FillTH1("hXic0PtFromBottom2",mcpart->Pt());
      fHistos->FillTH1("hNonPromptXicRap",mcpart->Y());
      }
    else{
      if(MCe->Charge()>0 && MCcasc->Charge()<0 && fabs(mcpart->Y())<0.5) fHistos->FillTH1("hXic0PtFromCharm1",mcpart->Pt());
      if(MCe->Charge()<0 && MCcasc->Charge()>0 && fabs(mcpart->Y())<0.5) fHistos->FillTH1("hXic0PtFromCharm2",mcpart->Pt());
...
void AliAnalysisTaskSEXic0Semileptonic::FillBottomContribution(AliAODMCParticle *mcpart, AliAODcascade *casc, AliAODTrack *trk){
	...
	if(lab_mother_of_Xic0>=0) mother_of_Xic0 = (AliAODMCParticle*) fMC->GetTrack(lab_mother_of_Xic0);
        ...
        if(elab>0 && Xilab>0){
          ...
          //if(In_Mass<=2.5 && cosoa>0 && StandardCutFlag(trk,casc,1,1,1,1)){
            fHistos->FillTH2("hXic0vseXiRPM",mcXic0_Pt,eXi); //not use
            //fHistos->FillTH2("hXic0vseXiRPM_2",mcXic0_Pt,eXi); 
            fHistos->FillTH1("hGenXic0Pt",mcXic0_Pt); //MC Pt of Xic0 not use
            fHistos->FillTH1("hRecoPairPt",eXi); //Reco Pt of eXi not use
            //fHistos->FillTH1("hGenXic0Pt_2",mcXic0_Pt); //MC Pt of Xic0
            //fHistos->FillTH1("hRecoPairPt_2",eXi); //Reco Pt of eXi
            if(eCharge>0 && XiCharge<0) fHistos->FillTH1("hGenXic0Pt1",mcXic0_Pt); //not use
            if(eCharge<0 && XiCharge>0) fHistos->FillTH1("hGenXic0Pt2",mcXic0_Pt); //not use

            if((TMath::Abs(mother_of_Xic0->GetPdgCode())==5132) || (TMath::Abs(mother_of_Xic0->GetPdgCode())==5232)) {
              fHistos->FillTH1("hGenXic0PtFromXib",mcXic0_Pt); //not use
              b_flag = 1;
            } else {
              fHistos->FillTH1("hGenXic0PtFromXic",mcXic0_Pt); //not use
              c_flag = 1;
            }
...
Bool_t AliAnalysisTaskSEXic0Semileptonic::StandardCutFlag(AliAODTrack *track, AliAODcascade *casc, Bool_t e_reco, Bool_t e_pid, Bool_t Xi_reco, Bool_t Xi_pid){
  //-------------------------------------eletron-------------------------------------
  if(e_reco){
...
//___________________________________________________________________
void AliAnalysisTaskSEXic0Semileptonic::FinishTaskOutput() {}
                //fOutput->Write();
//___________________________________________________________________
